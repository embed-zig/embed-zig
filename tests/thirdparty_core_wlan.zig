...
test "thirdparty/core_wlan/unit/std" {
    var t = glib.testing.T.new(gstd.runtime.std, gstd.runtime.time, .core_wlan_unit_std);
    defer t.deinit();
...
test "thirdparty/core_wlan/integration/embed_std" {
    var t = glib.testing.T.new(gstd.runtime.std, gstd.runtime.time, .core_wlan_integration_embed_std);
    defer t.deinit();
    t.run("integration", core_wlan.test_runner.integration.make(gstd.runtime));
    if (!t.wait()) return error.TestFailed;
}
