...
pub fn TestRunner() glib.testing.TestRunner {
    const Runner = struct {
        ...
        fn checkFixture(allocator: std.mem.Allocator) !void {
            ...
            try std.testing.expectEqualStrings("Example code for the `name-normalizer` output option", spec.info.title);
            try std.testing.expectEqualStrings("1.0.0", spec.info.version);
            try std.testing.expectEqual(@as(usize, 1), spec.paths.len);
            {
                const p_or = Spec.findNamed(Spec.PathItemOrRef, spec.paths, "/api/pets/{petId}") orelse return error.FixtureMismatch;
                const p_it = switch (p_or) {
                    .path_item => |x| x,
                    .reference => return error.FixtureMismatch,
                };
                const get_op = p_it.get orelse return error.FixtureMismatch;
                try std.testing.expectEqualStrings("getHttpPet", get_op.operation_id orelse return error.FixtureMismatch);
            ...
            const _sor_OneOf2things = Spec.findNamed(Spec.SchemaOrRef, components.schemas, "OneOf2things") orelse return error.FixtureMismatch;
            switch (_sor_OneOf2things) {
                .reference => |r| {
                    _ = r.ref_path;
                },
                .schema => |sch| {
