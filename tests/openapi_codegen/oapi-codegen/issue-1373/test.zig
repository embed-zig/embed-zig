...
pub fn TestRunner() glib.testing.TestRunner {
    const Runner = struct {
        ...
        fn checkFixture(allocator: std.mem.Allocator) !void {
            ...
            const _sor_RecursiveObject = Spec.findNamed(Spec.SchemaOrRef, components.schemas, "RecursiveObject") orelse return error.FixtureMismatch;
            switch (_sor_RecursiveObject) {
                .reference => |r| {
                    _ = r.ref_path;
                },
                .schema => |sch| {
                    try std.testing.expectEqual(@as(usize, 3), sch.all_of.len);
                },
            }
            const _sor_NonRecursiveObject = Spec.findNamed(Spec.SchemaOrRef, components.schemas, "NonRecursiveObject") orelse return error.FixtureMismatch;
            switch (_sor_NonRecursiveObject) {
                .reference => |r| {
                    _ = r.ref_path;
                },
                .schema => |sch| {
