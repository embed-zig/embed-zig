...
pub fn TestRunner() glib.testing.TestRunner {
    const Runner = struct {
        ...
        fn checkFixture(allocator: std.mem.Allocator) !void {
            ...
            try std.testing.expectEqualStrings("Issue 2031", spec.info.title);
            ...
            const _sor_ArrayContainer = Spec.findNamed(Spec.SchemaOrRef, components.schemas, "ArrayContainer") orelse return error.FixtureMismatch;
            switch (_sor_ArrayContainer) {
                .reference => |r| {
                    _ = r.ref_path;
                },
                .schema => |sch| {
