...
pub fn TestRunner() glib.testing.TestRunner {
    const Runner = struct {
        ...
        fn checkFixture(allocator: std.mem.Allocator) !void {
            ...
            try std.testing.expectEqual(@as(usize, 4), components.schemas.len);
            const _sor_GetPetsDto = Spec.findNamed(Spec.SchemaOrRef, components.schemas, "GetPetsDto") orelse return error.FixtureMismatch;
            switch (_sor_GetPetsDto) {
                .reference => |r| {
                    _ = r.ref_path;
                },
                .schema => |sch| {
                    ...
                    {
                        const pf = Spec.findNamed(Spec.SchemaOrRef, sch.properties, "data") orelse return error.FixtureMismatch;
                        const pSch = switch (pf) {
                            .schema => |x| x,
                            .reference => return error.FixtureMismatch,
                        };
                        try std.testing.expectEqual(@as(usize, 3), pSch.any_of.len);
                    }
                },
            }
