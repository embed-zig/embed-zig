...
pub fn TestRunner() glib.testing.TestRunner {
    const Runner = struct {
        ...
        fn checkFixture(allocator: std.mem.Allocator) !void {
            ...
            try std.testing.expectEqualStrings("Referencing an optional field, which has no information about the type it is will generate an `interface{}`, without the 'optional pointer'", spec.info.title);
