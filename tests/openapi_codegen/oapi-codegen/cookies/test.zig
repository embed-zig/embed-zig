...
const Handlers = struct {
    fn cookieParams(ctx_ptr: *anyopaque, req_ctx: Context, allocator: lib.mem.Allocator, args: ServerApi.operations.cookieParams.Args) !ServerApi.operations.cookieParams.Response {
        _ = req_ctx;
        _ = allocator;
...
fn run_generated_client_and_server_round_trip_cookie_parameters(t: *glib.testing.T, allocator: std.mem.Allocator) !void {
    _ = t;
    _ = allocator;
...
pub fn TestRunner(comptime phase: Phase) glib.testing.TestRunner {
    return switch (phase) {
        .spec => specRunner(),
        .roundtrip => glib.testing.TestRunner.fromFn(std, 1024 * 1024, run_generated_client_and_server_round_trip_cookie_parameters),
    };
}
