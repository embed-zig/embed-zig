...
pub fn TestRunner() glib.testing.TestRunner {
    const Runner = struct {
        ...
        fn checkFixture(allocator: std.mem.Allocator) !void {
            ...
            try std.testing.expectEqualStrings("Generate models", spec.info.title);
            try std.testing.expectEqualStrings("1.0.0", spec.info.version);
            try std.testing.expectEqual(@as(usize, 1), spec.paths.len);
            {
                const p_or = Spec.findNamed(Spec.PathItemOrRef, spec.paths, "/client") orelse return error.FixtureMismatch;
                const p_it = switch (p_or) {
                    .path_item => |x| x,
                    .reference => return error.FixtureMismatch,
                };
                const get_op = p_it.get orelse return error.FixtureMismatch;
                try std.testing.expectEqualStrings("getClient", get_op.operation_id orelse return error.FixtureMismatch);
                const put_op = p_it.put orelse return error.FixtureMismatch;
                try std.testing.expectEqualStrings("updateClient", put_op.operation_id orelse return error.FixtureMismatch);
            }
            ...
            const _sor_ClientType = Spec.findNamed(Spec.SchemaOrRef, components.schemas, "ClientType") orelse return error.FixtureMismatch;
            switch (_sor_ClientType) {
                .reference => |r| {
                    _ = r.ref_path;
                },
                .schema => |sch| {
            ...
            const _sor_Unreferenced = Spec.findNamed(Spec.SchemaOrRef, components.schemas, "Unreferenced") orelse return error.FixtureMismatch;
            switch (_sor_Unreferenced) {
                .reference => |r| {
                    _ = r.ref_path;
                },
                .schema => |sch| {
