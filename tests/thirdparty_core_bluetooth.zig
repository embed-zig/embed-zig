...
test "thirdparty/core_bluetooth/unit/std" {
    var t = glib.testing.T.new(gstd.runtime.std, gstd.runtime.time, .core_bluetooth_unit_std);
    defer t.deinit();
...
test "thirdparty/core_bluetooth/integration/std" {
    ...
    var t = glib.testing.T.new(gstd.runtime.std, gstd.runtime.time, .core_bluetooth_integration_std);
    defer t.deinit();
...
test "thirdparty/core_bluetooth/integration/embed_std" {
    var t = glib.testing.T.new(gstd.runtime.std, gstd.runtime.time, .core_bluetooth_integration_embed_std);
    defer t.deinit();
    t.run("integration", core_bluetooth.test_runner.integration.make(gstd.runtime));
    if (!t.wait()) return error.TestFailed;
}
