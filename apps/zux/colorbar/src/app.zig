...
pub const SpecType = blk: {
    ...
    builder.addSpecSlices(&.{
        ...
        @embedFile("spec/user_stories/boot_shows_split_7_colors.json"),
        @embedFile("spec/user_stories/split_click_targets_red.json"),
        @embedFile("spec/user_stories/red_click_targets_orange.json"),
        @embedFile("spec/user_stories/orange_click_targets_yellow.json"),
        @embedFile("spec/user_stories/yellow_click_targets_green.json"),
        @embedFile("spec/user_stories/green_click_targets_cyan.json"),
        @embedFile("spec/user_stories/cyan_click_targets_blue.json"),
        @embedFile("spec/user_stories/blue_click_targets_violet.json"),
        @embedFile("spec/user_stories/violet_click_wraps_split_7_colors.json"),
        @embedFile("spec/user_stories/idle_keeps_current_scene.json"),
        @embedFile("spec/user_stories/single_click_advances_once.json"),
        @embedFile("spec/user_stories/rapid_two_clicks_advance_two_scenes.json"),
    });
...
pub fn make(comptime platform_ctx: type, comptime platform_grt: type) type {
    return launcher.make(struct {
        ...
        pub const title = "colorbar";
        pub const description = "Boot button driven display colorbar test.";
        ...
        pub fn init(allocator: glib.std.mem.Allocator, base_config: ZuxApp.InitConfig) !*Self {
            ...
            init_config.scene_reducer = ZuxApp.ReducerHook.init(&self.reducers);
            init_config.scene_render = ZuxApp.RenderHook.init(&self.renders.scene);
        ...
        fn applyInitialState(init_config: *ZuxApp.InitConfig) void {
            init_config.initial_state = .{
                ...
                .scene = .{
                    .current = .split_7_colors,
                },
            };
        }

        fn uiRuntimeTaskOptions() glib.task.Options {
            if (@hasDecl(platform_ctx, "colorbarUiTaskOptions")) {
                return platform_ctx.colorbarUiTaskOptions();
            }
...
pub fn testRunner(comptime platform_grt: type) glib.testing.TestRunner {
    ...
    const UserStoryConfigFactoryImpl = struct {
        ...
        pub fn make(self_factory: *@This(), init_config: ZuxApp.InitConfig) !*Instance {
            ...
            self_factory.instance.init_config.scene_reducer = ZuxApp.ReducerHook.init(&self_factory.instance.reducers);
            self_factory.instance.init_config.scene_render = ZuxApp.RenderHook.init(&self_factory.instance.renders.scene);
...
pub fn run(comptime platform_ctx: type, comptime platform_grt: type) !void {
    ...
    t.run("colorbar/stories", Launcher.createTestRunner());
