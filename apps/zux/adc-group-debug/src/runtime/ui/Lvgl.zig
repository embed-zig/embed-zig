...
pub fn make(comptime grt: type, comptime ZuxAppType: type) type {
    ...
    const DebugState = @FieldType(ZuxAppType.Store.Stores, "debug").StateType;
    const Screen = ScreenMod.make(DebugState);
    ...
    const log = grt.std.log.scoped(.adc_group_debug_ui);
    ...
    return struct {
        ...
        fn renderOnce(self: *Runtime, lvgl_runtime: *LvglRuntimeType, render_state: *RenderState) !void {
            render_state.state = self.zux_app.store.stores.debug.get();
        ...
        fn render(render_state: *RenderState, runtime: *LvglRuntimeType, app: *Impl) !void {
            ...
            const state = render_state.state orelse app.store().stores.debug.get();
