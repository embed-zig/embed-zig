...
pub fn make(comptime platform_ctx: type, comptime platform_grt: type) type {
    return launcher.make(struct {
        ...
        pub const title = "task-smoke";
        pub const description = "Named glib.task Routine smoke test.";
    ...
    const Runner = struct {
        ...
        pub fn run(self: *@This(), t: *glib.testing.T, allocator: platform_grt.std.mem.Allocator) bool {
            ...
            runSmoke(platform_ctx, platform_grt) catch |err| {
                t.logErrorf("task smoke failed: {s}", .{@errorName(err)});
                return false;
            };
            return true;
        }
...
pub fn run(comptime platform_ctx: type, comptime platform_grt: type) !void {
    ...
    var t = glib.testing.T.new(platform_grt.std, platform_grt.time, .zux_task_smoke);
    defer t.deinit();

    t.run("task-smoke/routines", testRunner(platform_ctx, platform_grt));
...
fn runSmoke(comptime platform_ctx: type, comptime platform_grt: type) !void {
    ...
    const log = platform_grt.std.log.scoped(.zux_task_smoke);
    var alpha: TaskState = .{ .seed = 11 };
    var beta: TaskState = .{ .seed = 29 };

    const alpha_routine = glib.task.Routine.init(&alpha, runAlpha);
    const beta_routine = glib.task.Routine.init(&beta, runBeta);

    log.info("launching task smoke routines", .{});
    const alpha_handle = try platform_grt.task.go("zux/task_smoke/alpha", .{
        .min_stack_size = 4 * 1024,
    }, alpha_routine);
    const beta_handle = try platform_grt.task.go("zux/task_smoke/beta", .{
    ...
    if (!alpha.ran or alpha.output != 34) return error.AlphaTaskFailed;
    if (!beta.ran or beta.output != 87 or beta.scratch_checksum != 120) return error.BetaTaskFailed;
    log.info("task smoke routines passed: alpha={d}, beta={d}", .{ alpha.output, beta.output });
}

const TaskState = struct {
    seed: usize,
    output: usize = 0,
    scratch_checksum: usize = 0,
    ran: bool = false,
};

fn runAlpha(state: *TaskState) void {
    state.output = state.seed + 23;
    state.ran = true;
}

fn runBeta(state: *TaskState) void {
    var scratch: [16]u8 = undefined;
    for (&scratch, 0..) |*byte, i| {
        byte.* = @intCast(i);
    }
    ...
    state.scratch_checksum = checksum;
    state.output = state.seed * 3;
    state.ran = true;
}
