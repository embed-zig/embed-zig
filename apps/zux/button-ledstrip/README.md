...
- Given the LED strip is already in marquee mode, when the user holds `Power`, the LED strip turns off at the 3-second mark and stays off even if the same hold reaches 5 seconds.
...
- Given the LED strip is in marquee mode and has a target color, when the user waits 10ms, the LED strip interpolates once from the current color toward the target color; when the current color reaches the target color, the LED strip switches to the next target color in red, yellow, blue order and keeps interpolating.
- Given the LED strip has entered marquee mode from a 5-second hold, later long-press events from the same still-held press do not reset the in-flight marquee color progress.
- Given the LED strip has just looped from the red marquee stage to the yellow marquee stage, the next 10ms tick interpolates through an orange midpoint instead of jumping directly to yellow.
...
- `marquee_hold_5s_stays_off_after_3s`: Negative: Given the user starts holding `Power` while marquee is already active, the LED strip turns off at 3 seconds and does not re-enter marquee when the same hold reaches 5 seconds.
- `marquee_raw_hold_3s_turns_off`: Positive: Given marquee is already active, holding the raw `Power` button for 3 seconds emits a long-press gesture and turns the LED strip off through the same path used by desktop and device runtimes.
...
- `marquee_hold_after_5s_keeps_progress`: Negative: Given the LED strip has entered marquee mode from a 5-second hold and already made visible color progress, later long-press events from the same still-held press do not reset the visible color back to the start of the red transition.
...
- `marquee_close_to_red_targets_yellow`: Positive: Given the LED strip is in marquee mode and the current color has reached red, when the user waits for the next 10ms tick, the LED strip switches the target color to yellow and keeps interpolating.
- `marquee_red_to_yellow_lerps_after_loop_boundary`: Positive: Given the LED strip has just retargeted from red to yellow in marquee mode, when the user waits for the next 10ms tick, the visible color advances to an orange midpoint and render outputs that midpoint instead of jumping directly to yellow.
- `marquee_close_to_yellow_targets_blue`: Positive: Given the LED strip is in marquee mode and the current color has reached yellow, when the user waits for the next 10ms tick, the LED strip switches the target color to blue and keeps interpolating.
- `marquee_yellow_to_blue_lerps_after_loop_boundary`: Positive: Given the LED strip has just retargeted from yellow to blue in marquee mode, when the user waits for the next 10ms tick, the visible color advances to an intermediate color and render outputs that midpoint instead of jumping directly to blue.
...
- `marquee_blue_to_red_lerps_after_loop_boundary`: Positive: Given the LED strip has just retargeted from blue to red in marquee mode, when the user waits for the next 10ms tick, the visible color advances to a purple midpoint instead of jumping directly to red.
