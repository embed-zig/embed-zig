...
pub fn create(
    ...
    osal_mod.addImport("glib", b.modules.get("glib") orelse @panic("glib module missing"));
...
pub fn linkTest(
    ...
    if (!has_custom_config_header) {
        const osal = osal_library orelse createOsalLibrary(b);
        compile_test.linkLibrary(osal);
    }
}
...
fn createOsalLibrary(b: *std.Build) *std.Build.Step.Compile {
    ...
    const root_source = b.addWriteFiles().add("lvgl_osal_root.zig",
        \\const std = @import("std");
    ...
    osal.root_module.addImport("gstd", gstd_dep.module("gstd"));
