...
`lib/stdz` is the platform-facing runtime layer for `embed-zig`.
...
```zig
const stdz = @import("stdz").make(platform_impl);
```

Code written against `stdz` should, as much as possible, also work against
...
Today, `stdz` is not yet a full `std` replacement.
...
If `stdz` exposes `Thread`, `Thread.Mutex`, `log`, `posix`, `mem.Allocator`,
`crypto.Certificate`, or similar `std`-shaped APIs, code should be able to rely
on the same semantics.

At the same time, `stdz` may expose extra fields or types that help with
...
For this project, "drop-in replacement" does not mean that `stdz` must already
mirror every public symbol in `std`.

It means:

1. When `stdz` provides a `std`-shaped API, it should preserve `std`'s meaning.
2. `stdz` should keep moving toward broader `std` surface coverage.
...
`stdz` adds one extra capability on top of that model: injectable low-level
implementation.
...
That is the intended contract for `lib/stdz`.
