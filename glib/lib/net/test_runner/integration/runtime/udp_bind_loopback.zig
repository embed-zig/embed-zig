        ...
        pub fn run(self: *@This(), t: *testing_api.T, allocator: lib.mem.Allocator) bool {
            ...
            const Body = struct {
                ...
                fn recvFromWait(sock: *net.Runtime.Udp, buf: []u8, src: *net.netip.AddrPort) !usize {
                    while (true) {
                        return sock.recvFrom(buf, src) catch |err| switch (err) {
                            error.WouldBlock => {
                                _ = try sock.poll(.{ .read = true, .read_interrupt = true }, 1000);
                                continue;
                            },
                            else => return err,
                        };
                ...
                fn call() !void {
                    ...
                    try std.testing.expectEqualSlices(u8, &.{ 127, 0, 0, 1 }, &server_addr.addr().as4().?);
                    ...
                    try std.testing.expectEqual(request.len, req_sent);
                    ...
                    try std.testing.expectEqual(request.len, req_recv);
                    ...
                    try std.testing.expectEqual(response.len, resp_sent);
                    ...
                    try std.testing.expectEqual(response.len, resp_recv);
