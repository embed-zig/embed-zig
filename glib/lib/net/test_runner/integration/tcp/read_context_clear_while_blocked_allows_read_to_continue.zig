...
pub fn make(comptime std: type, comptime net: type) testing_api.TestRunner {
    const Runner = struct {
        ...
        pub fn run(self: *@This(), t: *testing_api.T, allocator: std.mem.Allocator) bool {
            _ = self;
            const Body = struct {
                fn waitUntilReadWaiting(conn: *net.TcpConn, comptime _: type) void {
                    while (true) {
                        conn.read_mu.lock();
                ...
                fn call(a: std.mem.Allocator) !void {
                    ...
                    const Worker = struct {
                        ...
                        fn write(ctx: *WriteCtx, comptime _: type) void {
                            // Write after the original read-context deadline would have expired.
                            net.time.sleep(60 * net.time.duration.MilliSecond);
                    ...
                    var io_ctx = try ctx_api.withDeadline(
                        ctx_api.background(),
                        @import("time").instant.add(ctx_api.now(), 30 * @import("time").duration.MilliSecond),
                    );
                    defer io_ctx.deinit();
