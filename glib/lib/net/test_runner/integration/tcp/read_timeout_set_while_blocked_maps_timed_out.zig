            ...
            const Body = struct {
                ...
                fn call(a: std.mem.Allocator) !void {
                    ...
                    const Worker = struct {
                        ...
                        fn closeLater(conn: net.Conn, comptime _: type) void {
                    ...
                    var close_thread = try Thread.spawn(.{}, Worker.closeLater, .{ server, std });

                    ready.waitUntilReady();
                    waitUntilReadWaiting(server_impl, std);
                    server.setReadDeadline(net.time.instant.add(net.time.instant.now(), 30 * net.time.duration.MilliSecond));
