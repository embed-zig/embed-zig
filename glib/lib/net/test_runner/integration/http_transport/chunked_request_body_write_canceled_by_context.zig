                    ...
                    try Utils.withServerState(
                        ...
                        struct {
                            fn run(conn: net.Conn, state: *WaitState) !void {
                                ...
                                try testing.expect(Utils.hasRequestLine(req_head, "POST /upload-chunked-cancel HTTP/1.1"));
                        ...
                        struct {
                            fn run(_: std.mem.Allocator, port: u16, state: *WaitState) !void {
                                ...
                                const url = try std.fmt.allocPrint(testing.allocator, "http://127.0.0.1:{d}/upload-chunked-cancel", .{port});
