                    ...
                    try Utils.withServerState(
                        ...
                        struct {
                            fn run(conn: net.Conn, _: *EmptyState) !void {
                                ...
                                try testing.expectEqualStrings("Basic ZXhwbGljaXQ=", Utils.headerValue(req_head, Http.Header.proxy_authorization) orelse "");
                        ...
                        struct {
                            fn run(_: std.mem.Allocator, port: u16, _: *EmptyState) !void {
                                const proxy_raw_url = try std.fmt.allocPrint(testing.allocator, "http://user:pass@127.0.0.1:{d}", .{port});
