                    ...
                    try Utils.withServerState(
                        ...
                        struct {
                            fn run(conn: net.Conn, _: *EmptyState) !void {
                                ...
                                try testing.expect(Utils.hasRequestLine(req_head, "GET /eof-response HTTP/1.1"));
                        ...
                        struct {
                            fn run(_: std.mem.Allocator, port: u16, _: *EmptyState) !void {
                                ...
                                const url = try std.fmt.allocPrint(testing.allocator, "http://127.0.0.1:{d}/eof-response", .{port});
