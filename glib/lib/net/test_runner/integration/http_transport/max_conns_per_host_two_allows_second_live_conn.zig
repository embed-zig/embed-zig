                ...
                fn call(a: std.mem.Allocator) !void {
                    ...
                    const accept_count = try Utils.withTwoRequestKeepAliveServer(testing.allocator, .{
                        ...
                        fn run(_: std.mem.Allocator, port: u16) !void {
                            ...
                            const url1 = try std.fmt.allocPrint(testing.allocator, "http://127.0.0.1:{d}/max-two-1", .{port});
                            defer testing.allocator.free(url1);
                            const url2 = try std.fmt.allocPrint(testing.allocator, "http://127.0.0.1:{d}/max-two-2", .{port});
