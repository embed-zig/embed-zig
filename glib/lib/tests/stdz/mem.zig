...
fn eqlIndexScalarCase(comptime lib: type) !void {
    if (!lib.mem.eql(u8, "stdz", "stdz")) return error.MemEqlEqualFailed;
    if (lib.mem.eql(u8, "stdz", "std")) return error.MemEqlDifferentFailed;

    const needle = lib.mem.indexOf(u8, "hello stdz world", "stdz") orelse return error.IndexOfFailed;
    if (needle != 6) return error.IndexOfWrongOffset;

    if (lib.mem.indexOf(u8, "hello stdz world", "zig") != null) return error.IndexOfUnexpectedMatch;
...
fn trimCase(comptime lib: type) !void {
    const trimmed = lib.mem.trim(u8, " \t  hello stdz \n", &lib.ascii.whitespace);
    if (!lib.mem.eql(u8, trimmed, "hello stdz")) return error.TrimFailed;
}
