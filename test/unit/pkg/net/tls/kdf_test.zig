...
test "hkdfExpandLabel basic" {
    const Crypto = runtime.std.Std.Crypto;

    const secret: [32]u8 = [_]u8{0x01} ** 32;
    const result = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "key", "", 16);
...
test "hkdfExpandLabel with context" {
    ...
    const result = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "s hs traffic", &context, 32);
...
test "hkdfExpandLabel different lengths" {
    ...
    const iv = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "iv", "", 12);
    try std.testing.expect(iv.len == 12);

    const key16 = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "key", "", 16);
    try std.testing.expect(key16.len == 16);

    const key32 = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "key", "", 32);
...
test "hkdfExpandLabel deterministic" {
    ...
    const r1 = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "key", "", 16);
    const r2 = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "key", "", 16);
...
test "hkdfExpandLabel different labels produce different output" {
    ...
    const r1 = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "key", "", 32);
    const r2 = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "iv", "", 32);
...
test "hkdfExpandLabel different secrets produce different output" {
    ...
    const r1 = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret1, "key", "", 16);
    const r2 = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret2, "key", "", 16);
    try std.testing.expect(!std.mem.eql(u8, &r1, &r2));
}
...
test "hkdfExpandLabel different contexts produce different output" {
    ...
    const r1 = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "key", &ctx1, 32);
    const r2 = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "key", &ctx2, 32);
    try std.testing.expect(!std.mem.eql(u8, &r1, &r2));
}
...
test "hkdfExpandLabel TLS 1.3 standard labels" {
    ...
    const client_hs = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "c hs traffic", &hash, 32);
    const server_hs = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "s hs traffic", &hash, 32);
    try std.testing.expect(!std.mem.eql(u8, &client_hs, &server_hs));

    const client_app = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "c ap traffic", &hash, 32);
    const server_app = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "s ap traffic", &hash, 32);
...
test "hkdfExpandLabel output is not all zeros" {
    ...
    const result = hkdfExpandLabel(Crypto.Hkdf.Sha256(), secret, "derived", "", 32);
