//! Embedded song catalog with simple square-wave renderer.
...
pub const catalog = [_]Song{
    .{ .id = "twinkle", .name = "Twinkle Star", .bpm = 100, .melody = &twinkle_melody, .bass = &twinkle_bass },
    .{ .id = "canon", .name = "Canon", .bpm = 60, .melody = &canon_melody, .bass = &canon_bass },
};

pub fn get(index: usize) Song {
    return catalog[index % catalog.len];
}
...
/// Render a voice as mono i16 PCM using square waves (no libm sin needed).
pub fn renderVoiceMono(allocator: std.mem.Allocator, bpm: u32, notes: []const Note, amp: f64, sample_rate: u32) ![]i16 {
    ...
    const amplitude: i16 = @intFromFloat(amp * 32767.0);
    ...
    for (notes) |n| {
        ...
        const half_period: usize = @intFromFloat(rate_f / (2.0 * n.freq_hz));
        if (half_period == 0) {
            @memset(out[write_idx .. write_idx + frames], 0);
            write_idx += frames;
            continue;
        ...
        for (0..frames) |i| {
            out[write_idx + i] = if ((i / half_period) % 2 == 0) amplitude else -amplitude;
        }
        write_idx += frames;
    }
    return out;
