...
pub fn make(comptime lib: type, comptime ChannelFactory: fn (type) type) type {
    const Channel = ChannelFactory([]u8);

    return struct {
        allocator: lib.mem.Allocator,
        left_to_right: Channel,
        right_to_left: Channel,
        ...
        pub fn init(allocator: lib.mem.Allocator) !Self {
            return .{
                .allocator = allocator,
                .left_to_right = try Channel.make(allocator, 64),
                .right_to_left = try Channel.make(allocator, 64),
            };
        }
