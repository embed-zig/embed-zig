//! Shared helpers and cases for fd `Packet` integration tests.
...
pub fn Suite(comptime lib: type) type {
    ...
    return struct {
        ...
        pub fn packetIpv4Loopback() !void {
        ...
        pub fn packetIpv6Loopback() !void {
        ...
        pub fn packetConnectedReadWrite() !void {
        ...
        pub fn packetConnectContextLoopback(a: Allocator) !void {
        ...
        pub fn packetConnectContextCanceledBeforeStart(a: Allocator) !void {
            var ctx_api = try Context.init(a);
            defer ctx_api.deinit();
        ...
        pub fn packetConnectContextDeadlineExceededBeforeStart(a: Allocator) !void {
            var ctx_api = try Context.init(a);
            defer ctx_api.deinit();
        ...
        pub fn packetConnectContextCanceledDuringConnect(a: Allocator) !void {
            var ctx_api = try Context.init(a);
            defer ctx_api.deinit();
        ...
        pub fn packetConnectContextDeadlineExceededDuringConnect(a: Allocator) !void {
            var ctx_api = try Context.init(a);
            defer ctx_api.deinit();
        ...
        pub fn packetPreservesDatagramBoundaries() !void {
            var receiver = try bindLoopback(AddrPort.from4(.{ 127, 0, 0, 1 }, 0));
            defer receiver.deinit();
            var sender = try bindLoopback(AddrPort.from4(.{ 127, 0, 0, 1 }, 0));
            defer sender.deinit();
        ...
        pub fn packetReadDeadlineTimesOut() !void {
        ...
        pub fn packetReadDeadlineClearAllowsLaterRead() !void {
            var receiver = try bindLoopback(AddrPort.from4(.{ 127, 0, 0, 1 }, 0));
            defer receiver.deinit();
            var sender = try bindLoopback(AddrPort.from4(.{ 127, 0, 0, 1 }, 0));
            defer sender.deinit();
        ...
        pub fn packetFullDuplexStreaming() !void {
        ...
        pub fn packetOpsAfterCloseReturnClosed() !void {
            var packet = try Packet.initSocket(posix.AF.INET);
            packet.close();
        ...
        pub fn packetCloseIsIdempotent() !void {
            var packet = try Packet.initSocket(posix.AF.INET);
            packet.close();
            packet.close();
