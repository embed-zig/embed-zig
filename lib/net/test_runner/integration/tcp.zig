...
fn runImpl(comptime lib: type, t: *testing_api.T, alloc: lib.mem.Allocator) !void {
    ...
    const Runner = struct {
        ...
        fn allocatorAlignment(comptime any_lib: type) type {
        ...
        fn OneShotAllocatorType(comptime any_lib: type) type {
        ...
        fn tcpListenerAcceptReportsOutOfMemory() !void {
            const OneShotAllocator = OneShotAllocatorType(lib);

            var allocator = OneShotAllocator{ .backing = testing.allocator };
            var ln = try Net.TcpListener.init(allocator.allocator(), .{
                .address = addr4(.{ 127, 0, 0, 1 }, 0),
            });
            defer ln.deinit();
            try ln.listen();
            ...
            try testing.expectError(error.OutOfMemory, ln.accept());
        }
    ...
    try Runner.tcpListenerAcceptReportsOutOfMemory();
