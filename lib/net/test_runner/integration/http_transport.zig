...
pub fn make(comptime lib: type) testing_api.TestRunner {
    const Runner = struct {
        ...
        pub fn run(self: *@This(), t: *testing_api.T, allocator: lib.mem.Allocator) bool {
            _ = self;
            _ = allocator;
            t.run("local_returns_200", local_returns_200.make(lib));
            t.run("local_returns_404", local_returns_404.make(lib));
            t.run("default_user_agent_matches", default_user_agent_matches.make(lib));
            t.run("empty_user_agent_suppresses_default", empty_user_agent_suppresses_default.make(lib));
            t.run("context_deadline_exceeded", context_deadline_exceeded.make(lib));
            t.run("response_header_timeout_exceeded", response_header_timeout_exceeded.make(lib));
            t.run("response_header_timeout_does_not_limit_body_read", response_header_timeout_does_not_limit_body_read.make(lib));
            t.run("response_body_read_canceled_by_context", response_body_read_canceled_by_context.make(lib));
            t.run("response_body_read_deadline_exceeded_by_context", response_body_read_deadline_exceeded_by_context.make(lib));
            t.run("request_body_write_canceled_by_context", request_body_write_canceled_by_context.make(lib));
            t.run("request_body_write_deadline_exceeded_by_context", request_body_write_deadline_exceeded_by_context.make(lib));
            t.run("chunked_request_body_write_canceled_by_context", chunked_request_body_write_canceled_by_context.make(lib));
            t.run("chunked_request_body_write_deadline_exceeded_by_context", chunked_request_body_write_deadline_exceeded_by_context.make(lib));
            t.run("configured_max_header_bytes_allows_large_response_headers", configured_max_header_bytes_allows_large_response_headers.make(lib));
            t.run("response_body_larger_than_max_body_bytes_fails", response_body_larger_than_max_body_bytes_fails.make(lib));
            t.run("default_max_body_bytes_allows_large_response", default_max_body_bytes_allows_large_response.make(lib));
            t.run("large_response_streams_without_buffering_whole_body", large_response_streams_without_buffering_whole_body.make(lib));
            t.run("default_max_body_bytes_allows_large_request", default_max_body_bytes_allows_large_request.make(lib));
            t.run("large_request_streams_without_buffering_whole_body", large_request_streams_without_buffering_whole_body.make(lib));
            t.run("connect_method_is_rejected", connect_method_is_rejected.make(lib));
            t.run("https_connect_proxy_auth_required", https_connect_proxy_auth_required.make(lib));
            t.run("https_connect_proxy_auth_required_with_body", https_connect_proxy_auth_required_with_body.make(lib));
            t.run("https_connect_proxy_rejected", https_connect_proxy_rejected.make(lib));
            t.run("https_connect_proxy_rejected_with_body", https_connect_proxy_rejected_with_body.make(lib));
            t.run("https_connect_proxy_response_header_timeout", https_connect_proxy_response_header_timeout.make(lib));
            t.run("https_connect_proxy_tls_init_failure_closes_tunnel_conn", https_connect_proxy_tls_init_failure_closes_tunnel_conn.make(lib));
            t.run("https_proxy_userinfo_generates_proxy_authorization", https_proxy_userinfo_generates_proxy_authorization.make(lib));
            t.run("https_proxy_invalid_percent_encoding_is_rejected", https_proxy_invalid_percent_encoding_is_rejected.make(lib));
            t.run("https_proxy_oversized_userinfo_is_rejected", https_proxy_oversized_userinfo_is_rejected.make(lib));
            t.run("https_proxy_connect_headers_override_url_userinfo", https_proxy_connect_headers_override_url_userinfo.make(lib));
            t.run("idle_connection_is_reused", idle_connection_is_reused.make(lib));
            t.run("disable_keep_alives_forces_new_conn", disable_keep_alives_forces_new_conn.make(lib));
            t.run("max_idle_conns_one_keeps_only_one_idle_conn_across_hosts", max_idle_conns_one_keeps_only_one_idle_conn_across_hosts.make(lib));
            t.run("max_idle_conns_per_host_one_keeps_only_one_idle_conn", max_idle_conns_per_host_one_keeps_only_one_idle_conn.make(lib));
            t.run("close_idle_connections_forces_new_conn", close_idle_connections_forces_new_conn.make(lib));
            t.run("early_response_body_close_does_not_reuse_conn", early_response_body_close_does_not_reuse_conn.make(lib));
            t.run("idle_connection_timeout_forces_new_conn", idle_connection_timeout_forces_new_conn.make(lib));
            t.run("same_host_request_while_body_open_uses_second_conn", same_host_request_while_body_open_uses_second_conn.make(lib));
            t.run("max_conns_per_host_one_blocks_second_request_until_first_response_closes", max_conns_per_host_one_blocks_second_request_until_first_response_closes.make(lib));
            t.run("max_conns_per_host_two_allows_second_live_conn", max_conns_per_host_two_allows_second_live_conn.make(lib));
            t.run("max_conns_per_host_waiter_reuses_returned_idle_conn", max_conns_per_host_waiter_reuses_returned_idle_conn.make(lib));
            t.run("max_conns_per_host_waiter_deadline_exceeded", max_conns_per_host_waiter_deadline_exceeded.make(lib));
            t.run("max_conns_per_host_waiter_canceled", max_conns_per_host_waiter_canceled.make(lib));
            t.run("close_idle_connections_with_max_conns_per_host_does_not_leak_capacity", close_idle_connections_with_max_conns_per_host_does_not_leak_capacity.make(lib));
            t.run("chunked_request_uses_transfer_encoding", chunked_request_uses_transfer_encoding.make(lib));
            t.run("chunked_response_streams", chunked_response_streams.make(lib));
            t.run("eof_delimited_response_streams", eof_delimited_response_streams.make(lib));
            t.run("head_response_is_bodyless", head_response_is_bodyless.make(lib));
            t.run("status_204_response_is_bodyless", status_204_response_is_bodyless.make(lib));
            t.run("status_304_response_is_bodyless", status_304_response_is_bodyless.make(lib));
            t.run("informational_continue_then_final_response", informational_continue_then_final_response.make(lib));
            t.run("expect_continue_timeout_sends_body_without_informational", expect_continue_timeout_sends_body_without_informational.make(lib));
            t.run("final_response_without_continue_skips_request_body", final_response_without_continue_skips_request_body.make(lib));
            t.run("request_body_streams_before_round_trip_completes", request_body_streams_before_round_trip_completes.make(lib));
            t.run("response_body_streams_progressively", response_body_streams_progressively.make(lib));
            t.run("full_duplex_request_and_response", full_duplex_request_and_response.make(lib));
            t.run("bodyless_early_response_does_not_wait_for_blocked_request_body", bodyless_early_response_does_not_wait_for_blocked_request_body.make(lib));
            t.run("stale_idle_connection_retries_replayable_get", stale_idle_connection_retries_replayable_get.make(lib));
            t.run("stale_idle_connection_retries_idempotent_replayable_post", stale_idle_connection_retries_idempotent_replayable_post.make(lib));
            return t.wait();
        }
