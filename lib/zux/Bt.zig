...
test "zux/Bt/unit_tests/set_and_emit_reports_through_event_receiver" {
    ...
    const DummyCentral = struct {
        ...
        pub fn discoverServices(_: *@This(), _: u16, _: []bt.Central.DiscoveredService) bt.Central.GattError!usize { return 0; }
        pub fn discoverChars(_: *@This(), _: u16, _: u16, _: u16, _: []bt.Central.DiscoveredChar) bt.Central.GattError!usize { return 0; }
        pub fn gattRead(_: *@This(), _: u16, _: u16, _: []u8) bt.Central.GattError!usize { return 0; }
        ...
        pub fn exchangeMtu(_: *@This(), _: u16, mtu: u16) bt.Central.GattError!u16 { return mtu; }
        ...
        pub fn getAttMtu(_: *@This(), _: u16) u16 { return bt.Central.DEFAULT_ATT_MTU; }
        pub fn getState(_: *@This()) bt.Central.State { return .idle; }
        ...
        pub fn getAddr(_: *@This()) ?bt.Central.BdAddr { return null; }
    ...
    const DummyPeripheral = struct {
        ...
        pub fn disconnect(_: *@This(), _: u16) void {}
        pub fn getState(_: *@This()) bt.Peripheral.State { return .idle; }
        ...
        pub fn getAddr(_: *@This()) ?bt.Peripheral.BdAddr { return null; }
        pub fn deinit(_: *@This()) void {}
    };
    ...
    try std.testing.expectEqual(@as(usize, 1), sink.central_connected_count);
    try std.testing.expectEqual(@as(usize, 1), sink.central_disconnected_count);
    try std.testing.expectEqual(@as(usize, 1), sink.central_notification_count);
    try std.testing.expectEqual(@as(usize, 1), sink.periph_connected_count);
    try std.testing.expectEqual(@as(usize, 1), sink.periph_disconnected_count);
    try std.testing.expectEqual(@as(usize, 1), sink.periph_mtu_changed_count);
    try std.testing.expectEqual(@as(u32, 41), sink.last_source_id);
    try std.testing.expectEqual(@as(usize, 10), sink.last_name_len);
    try std.testing.expectEqual(@as(usize, 7), sink.last_adv_len);
    try std.testing.expectEqual(@as(i8, -52), sink.last_rssi);
    try std.testing.expectEqual(@as(u16, 0x0040), sink.last_central_conn_handle);
    try std.testing.expectEqual(@as(u16, 0x0041), sink.last_periph_conn_handle);
    try std.testing.expectEqual(@as(u16, 0x0009), sink.last_notification_attr_handle);
    try std.testing.expectEqual(@as(usize, 3), sink.last_notification_len);
    try std.testing.expectEqual(@as(u16, 247), sink.last_periph_mtu);

    bt_adapter.clearEventReceiver();
    try std.testing.expect(host_impl.receiver_ctx == null);
    try std.testing.expect(host_impl.emit_fn == null);
}

test "zux/Bt/integration_tests/init_adapts_mocker_host_updates" {
    const std = @import("std");

    const World = bt.Mocker(std);
    ...
    var scanner_bt = Bt.init(scanner_host);
    defer scanner_bt.clearEventReceiver();
    scanner_bt.setEventReceiver(&receiver);

    var advertiser_bt = Bt.init(advertiser_host);
    ...
    try std.testing.expectEqual(@as(usize, 1), sink.started_count);
    try std.testing.expectEqual(@as(usize, 1), sink.stopped_count);
    try std.testing.expect(sink.found_count >= 1);
    try std.testing.expectEqual(@as(u32, 22), sink.last_started_source_id);
    try std.testing.expectEqual(@as(u32, 22), sink.last_stopped_source_id);
    try std.testing.expectEqual(@as(u32, 11), sink.last_found_source_id);
}
