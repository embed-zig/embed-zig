    ...
    return struct {
        ...
        pub fn onLedStrip(stores: *BuiltApp.Store.Stores) void {
            ...
            const expected_colors = [_]ledstrip.Color{
                ...
                ledstrip.Color.rgb(112, 0, 16),
                ledstrip.Color.rgb(96, 0, 32),
                ledstrip.Color.rgb(80, 0, 48),
                ledstrip.Color.rgb(64, 0, 64),
                ledstrip.Color.rgb(48, 0, 80),
                ledstrip.Color.rgb(32, 0, 96),
                ledstrip.Color.rgb(16, 0, 112),
                ledstrip.Color.rgb(0, 0, 128),
                ledstrip.Color.rgb(16, 0, 112),
                ledstrip.Color.rgb(32, 0, 96),
                ledstrip.Color.rgb(48, 0, 80),
                ledstrip.Color.rgb(64, 0, 64),
        ...
        fn driveSequence(app: *BuiltApp) !void {
            try app.set_led_strip_pixels(.strip, BuiltApp.FrameType.solid(ledstrip.Color.red), 128);
            try waitForCallbackCount(1);

            const duration_ns = 8 * lib.time.ns_per_ms;
            const interval_ns = 8 * lib.time.ns_per_ms;
            try app.set_led_strip_pingpong(
                .strip,
                BuiltApp.FrameType.solid(ledstrip.Color.red),
                BuiltApp.FrameType.solid(ledstrip.Color.blue),
                128,
                duration_ns,
                interval_ns,
            );
