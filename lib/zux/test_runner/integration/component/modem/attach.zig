...
fn TestCase(comptime lib: type, comptime BuiltApp: type) type {
    return struct {
        ...
        const expected_callback_count = 6;
        ...
        pub fn onModem(stores: *BuiltApp.Store.Stores) void {
            ...
            switch (callback_count) {
                1 => checkState(state, .locked, .offline, .detached),
                2 => checkState(state, .ready, .offline, .detached),
                3 => checkState(state, .ready, .searching, .detached),
                4 => checkState(state, .ready, .home, .detached),
                5 => checkState(state, .ready, .home, .attaching),
                6 => checkState(state, .ready, .home, .connected),
                else => fail(.unexpected_callback_count),
            }
        }

        fn driveSequence(app: *BuiltApp) !void {
            try app.modem_sim_state_changed(.cell, .locked);
