    ...
    return struct {
        ...
        pub fn onModem(stores: *BuiltApp.Store.Stores) void {
            ...
            switch (callback_count) {
                1 => checkState(state, .unknown, .offline, .detached, -83, 6, .lte, ""),
                2 => checkState(state, .ready, .offline, .detached, -83, 6, .lte, ""),
                3 => checkState(state, .ready, .searching, .detached, -83, 6, .lte, ""),
                4 => checkState(state, .ready, .searching, .detached, -83, 6, .lte, "bootstrap.prod"),
                5 => checkState(state, .ready, .searching, .attaching, -83, 6, .lte, "bootstrap.prod"),
                6 => checkState(state, .ready, .home, .attaching, -83, 6, .lte, "bootstrap.prod"),
                7 => checkState(state, .ready, .home, .attaching, -67, 1, .lte_m, "bootstrap.prod"),
                8 => checkState(state, .ready, .home, .connected, -67, 1, .lte_m, "bootstrap.prod"),
                else => fail(.unexpected_callback_count),
            }
        }

        fn driveSequence(app: *BuiltApp) !void {
            try app.modem_network_signal_changed(.cell, common.signal(-83, 6, .lte));
            try waitForCallbackCount(1);

            try app.modem_sim_state_changed(.cell, .ready);
            try waitForCallbackCount(2);

            try app.modem_network_registration_changed(.cell, .searching);
            try waitForCallbackCount(3);

            try app.modem_data_apn_changed(.cell, "bootstrap.prod");
            try waitForCallbackCount(4);

            try app.modem_data_packet_state_changed(.cell, .attaching);
            try waitForCallbackCount(5);

            try app.modem_network_registration_changed(.cell, .home);
            try waitForCallbackCount(6);

            try app.modem_network_signal_changed(.cell, common.signal(-67, 1, .lte_m));
            try waitForCallbackCount(7);

            try app.modem_data_packet_state_changed(.cell, .connected);
            try waitForCallbackCount(8);
        }

        fn checkState(
            state: component_modem.State,
            expected_sim: component_modem.SimState,
            expected_registration: component_modem.RegistrationState,
            expected_packet: component_modem.PacketState,
            expected_rssi_dbm: i16,
            expected_ber: ?u8,
            expected_rat: component_modem.Rat,
            expected_apn: []const u8,
        ) void {
            if (state.source_id != 51) {
                fail(.wrong_source_id);
                return;
            }
            if (state.sim != expected_sim) {
                fail(.wrong_sim);
                return;
            }
            ...
            if (state.packet != expected_packet) {
                fail(.wrong_packet);
                return;
            }
