...
const ServeMux = @import("bt/host/server/ServeMux.zig");
const Receiver = @import("bt/host/server/Receiver.zig");
...
pub fn make(comptime lib: type, comptime Channel: fn (type) type) type {
    return struct {
        ...
        pub const ServeMux = bt.ServeMux.make(lib, self.Server);
        pub const Receiver = bt.Receiver.make(lib, self.Server);
    };
}
