...
const binding = @import("bindings/wifi_sta.zig");
...
pub fn init(self: *WifiSta) !void {
    try check("devkit_wifi_sta_init", binding.devkit_wifi_sta_init());
    binding.devkit_wifi_sta_set_event_handler(self, dispatchEvent);
}
...
pub fn startScan(self: *WifiSta, config: Sta.ScanConfig) Sta.ScanError!void {
    ...
    const rc = binding.devkit_wifi_sta_start_scan(
        @ptrCast(ssid.ptr),
        ssid.len,
        config.channel,
        config.show_hidden,
...
pub fn disconnect(self: *WifiSta) void {
    _ = self;
    binding.devkit_wifi_sta_disconnect();
}

pub fn getState(self: *WifiSta) Sta.State {
    _ = self;
    return switch (binding.devkit_wifi_sta_state()) {
...
pub fn deinit(self: *WifiSta) void {
    binding.devkit_wifi_sta_set_event_handler(null, null);
    self.hook_ctx = null;
    self.hook_cb = null;
...
fn check(call_name: []const u8, rc: c_int) !void {
    ...
    esp.grt.std.log.scoped(.devkit_wifi_sta).err("{s} failed with rc={d}", .{ call_name, rc });
