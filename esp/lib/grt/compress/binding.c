...
enum {
    ...
    ESPZ_COMPRESS_CALLBACK_ERROR = -6,
};

typedef int (*espz_compress_stream_callback)(const uint8_t *data, size_t len, void *ctx);

struct espz_compress_callback_state {
    espz_compress_stream_callback callback;
    void *ctx;
    size_t written;
    int callback_failed;
};

static int espz_compress_put_buf(const void *buf, int len, void *user)
{
    struct espz_compress_callback_state *state = (struct espz_compress_callback_state *)user;
    if (len < 0) {
        return 0;
    }
    if (len == 0) {
    ...
    const int ok = state->callback((const uint8_t *)buf, (size_t)len, state->ctx);
    if (!ok) {
        state->callback_failed = 1;
        return 0;
    }
    state->written += (size_t)len;
    return 1;
}
...
int espz_compress_inflate_stream(
    int container,
    const uint8_t *compressed,
    size_t compressed_len,
    espz_compress_stream_callback callback,
    ...
    size_t *written)
{
    if ((compressed == NULL && compressed_len != 0) || callback == NULL || written == NULL) {
        return ESPZ_COMPRESS_UNEXPECTED;
    }
    if (container == ESPZ_COMPRESS_CONTAINER_GZIP) {
        return ESPZ_COMPRESS_UNSUPPORTED;
    ...
    struct espz_compress_callback_state state = {
        .callback = callback,
        .ctx = ctx,
        .written = 0,
        .callback_failed = 0,
    ...
    tinfl_decompressor *decomp = calloc(1, sizeof(tinfl_decompressor));
    uint8_t *out = malloc(TINFL_LZ_DICT_SIZE);
    if (decomp == NULL || out == NULL) {
        free(decomp);
        free(out);
        return ESPZ_COMPRESS_UNEXPECTED;
    }
    ...
    size_t in_pos = 0;
    const mz_uint32 flags = (container == ESPZ_COMPRESS_CONTAINER_ZLIB ? TINFL_FLAG_PARSE_ZLIB_HEADER : 0);

    while (1) {
        size_t in_bytes = compressed_len - in_pos;
        size_t out_bytes = TINFL_LZ_DICT_SIZE;
        tinfl_status status = tinfl_decompress(
            decomp,
            compressed + in_pos,
            &in_bytes,
        ...
        in_pos += in_bytes;
        if (out_bytes != 0) {
            if (!espz_compress_put_buf(out, (int)out_bytes, &state)) {
                free(decomp);
                free(out);
                return ESPZ_COMPRESS_CALLBACK_ERROR;
            }
        }
        ...
        if (status < TINFL_STATUS_DONE) {
            free(decomp);
            free(out);
            return state.callback_failed ? ESPZ_COMPRESS_CALLBACK_ERROR : ESPZ_COMPRESS_INVALID_DATA;
        }
        if (in_bytes == 0 && out_bytes == 0) {
            free(decomp);
            free(out);
