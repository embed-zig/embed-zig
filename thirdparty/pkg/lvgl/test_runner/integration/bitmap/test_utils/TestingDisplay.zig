...
pub fn init(allocator: Allocator, width_px: u16, height_px: u16, output: ?*Display) Self {
    return .{
        .allocator = allocator,
        .width_px = width_px,
        .height_px = height_px,
...
pub fn initPassthrough(allocator: Allocator, width_px: u16, height_px: u16, output: ?*Display) Self {
    var self = init(allocator, width_px, height_px, output);
    self.strict = false;
    return self;
}
...
pub fn display(self: *Self, comptime grt: type) !Display {
    return Display.make(grt, Adapter).init(.{
...
pub fn assertComplete(self: *const Self) Error!void {
    ...
    if (self.strict and self.next_result != self.results.items.len) {
...
fn recordDraw(
    ...
    if (self.next_result >= self.results.items.len) {
        if (self.output) |output| {
            output.drawBitmap(x, y, w, h, pixels) catch |err| {
                self.failure = err;
                return err;
            };
        }
        ...
        pub fn run(self: *@This(), t: *glib.testing.T, allocator: glib.std.mem.Allocator) bool {
            ...
            const Cases = struct {
                fn comparesExpectedDrawsInOrder(alloc: std.mem.Allocator) !void {
                    var testing_display = Self.init(alloc, 8, 4, null);
                    ...
                    var output = try testing_display.display(grt);
                    defer output.deinit();
                ...
                fn passthroughModeAllowsUnspecifiedDraws(alloc: std.mem.Allocator) !void {
                    var testing_display = Self.initPassthrough(alloc, 8, 4, null);
                    defer testing_display.deinit();
            ...
            Cases.passthroughModeAllowsUnspecifiedDraws(alloc) catch |err| {
                t.logFatal(@errorName(err));
                return false;
            };
            return true;
