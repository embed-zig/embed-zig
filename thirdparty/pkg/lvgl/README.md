# thirdparty/pkg/lvgl — LVGL binding notes

[![CI](https://github.com/embed-zig/lvgl/actions/workflows/ci.yml/badge.svg)](https://github.com/embed-zig/lvgl/actions/workflows/ci.yml)

This document describes the intended layering for `thirdparty/pkg/lvgl`.
...
Today `thirdparty/pkg/lvgl` is past the bootstrap stage.

Implemented so far:

- LVGL `v9.5.0` sources are built by `thirdparty/build/pkg/lvgl.zig`
...
- isolated `thirdparty/pkg/lvgl` tests cover the currently implemented layers
...
- `thirdparty/build/pkg/lvgl.zig`
- `thirdparty/pkg/lvgl/config.default.h`
- `src/binding.c`
- `src/binding.zig`
...
In this repo, `thirdparty/build/pkg/lvgl.zig` selects one complete header source:

- the shipped default at `thirdparty/pkg/lvgl/config.default.h`
...
`thirdparty/build/pkg/lvgl.zig` applies the same selected `lv_conf.h` to the `lvgl`
...
`thirdparty/pkg/lvgl/include/lv_os_custom.h` and `thirdparty/pkg/lvgl_osal.zig`.
...
- ship a checked-in default header at `thirdparty/pkg/lvgl/config.default.h`
...
- expose a curated set of build options in `thirdparty/build/pkg/lvgl.zig`
