//! xfer.recv — shared receive-side transfer loop.
//!
//! The concrete transport is supplied by the caller and must provide one
//! bidirectional session surface:
//! - `read(timeout_ms, out)` to wait for one inbound control/data packet
...
pub fn recv(
    ...
    const Transport = switch (@typeInfo(TransportPtr)) {
        ...
        else => @compileError("xfer.recv expects a transport pointer"),
    };
    ...
    while (true) {
        const payload_len = transport.read(config.timeout_ms, &chunk_buf) catch |err| switch (err) {
            error.Timeout => {
                ...
                if (initialized) {
                    try sendMissingWriteChunks(
                        transport,
                        rcvmask[0..Chunk.Bitmask.requiredBytes(total)],
                        total,
                        max_chunk_msg,
                    );
                }
        ...
        if (!started) {
            if (!Chunk.isWriteStartMagic(payload)) return error.InvalidWriteStart;
            started = true;
            continue;
        }

        if (Chunk.isWriteStartMagic(payload)) continue;
...
fn sendMissingWriteChunks(transport: anytype, rcvmask: []const u8, total: u16, max_chunk_msg: usize) !void {
    ...
    const TestCase = struct {
        fn run() !void {
            const HappyTransport = struct {
                start: [Chunk.write_start_magic.len]u8 = Chunk.write_start_magic,
            ...
            const payload_bytes = try recv(grt, grt.std.testing.allocator, &transport, .{
                .att_mtu = 9,
            });
            ...
            try grt.std.testing.expectEqual(@as(usize, 1), transport.write_count);
            try grt.std.testing.expectEqualSlices(u8, &Chunk.ack_signal, transport.writes[0][0..transport.write_lens[0]]);
            ...
            const retry_payload = try recv(grt, grt.std.testing.allocator, &retry_transport, .{
                .att_mtu = 9,
            });
            ...
            try grt.std.testing.expectEqual(@as(usize, 2), retry_transport.write_count);
            try grt.std.testing.expectEqualSlices(u8, expected, retry_transport.writes[0][0..retry_transport.write_lens[0]]);
            try grt.std.testing.expectEqualSlices(u8, &Chunk.ack_signal, retry_transport.writes[1][0..retry_transport.write_lens[1]]);
        }
    };
    const Runner = struct {
