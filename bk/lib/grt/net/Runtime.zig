...
const ThreadCommon = @import("../std/ThreadCommon.zig");

const runtime = glib.net.runtime;
const netip = glib.net.netip;
const max_udp_payload_len = glib.std.math.maxInt(u16);

const Mutex = ThreadCommon.Mutex;
const Condition = ThreadCommon.Condition;

var registry_mutex: Mutex = .{};
var registry_head: ?*State = null;
...
const State = struct {
    ...
    fn create(conn: *binding.netconn) error{OutOfMemory}!*State {
        const self = heap.allocator.create(State) catch return error.OutOfMemory;
        self.* = .{ .conn = conn };
        registerState(self);
        return self;
    }
    ...
    fn destroy(self: *State) void {
        if (self.pending_recv) |buf| {
            binding.bkz_lwip_netbuf_delete(buf);
        }
...
pub const Tcp = struct {
    ...
    pub fn close(self: *Self) void {
        ...
        if (self.state.conn) |conn| {
            _ = binding.bkz_lwip_netconn_close(conn);
        }
    }
    ...
    pub fn shutdown(self: *Self, how: runtime.ShutdownHow) runtime.SocketError!void {
        ...
        return switch (binding.bkz_lwip_netconn_shutdown(conn, shut_rx, shut_tx)) {
            binding.err_ok => {},
    ...
    pub fn bind(self: *Self, addr: netip.AddrPort) runtime.SocketError!void {
        ...
        return switch (binding.bkz_lwip_netconn_bind(conn, &encoded, addr.port())) {
            binding.err_ok => {},
    ...
    pub fn listen(self: *Self, backlog: u31) runtime.SocketError!void {
        ...
        return switch (binding.bkz_lwip_netconn_listen(conn, capped_backlog)) {
            binding.err_ok => {},
            binding.err_use => error.AddressInUse,
    ...
    pub fn accept(self: *Self, remote: ?*netip.AddrPort) runtime.SocketError!Tcp {
        ...
        const rc = binding.bkz_lwip_netconn_accept(conn, &accepted);
        switch (rc) {
            binding.err_ok => {},
        ...
        errdefer _ = binding.bkz_lwip_netconn_delete(accepted);
        binding.bkz_lwip_netconn_set_nonblocking(accepted, 1);
    ...
    pub fn finishConnect(self: *Self) runtime.SocketError!void {
        const conn = try liveConn(self.state);
        const err = binding.bkz_lwip_netconn_err(conn);
    ...
    pub fn send(self: *Self, buf: []const u8) runtime.SocketError!usize {
        ...
        var written: usize = 0;
        return switch (binding.bkz_lwip_netconn_write(conn, buf.ptr, buf.len, &written)) {
    ...
    pub fn setOpt(self: *Self, opt: runtime.TcpOption) runtime.SetSockOptError!void {
        ...
        switch (opt) {
            ...
            .tcp => |tcp_opt| switch (tcp_opt) {
                .no_delay => |enabled| try setOptResult(binding.bkz_lwip_netconn_set_tcp_no_delay(conn, if (enabled) 1 else 0)),
            },
        }
    }
...
pub const Udp = struct {
    ...
    pub fn send(self: *Self, buf: []const u8) runtime.SocketError!usize {
        ...
        return switch (binding.bkz_lwip_netconn_send(conn, buf.ptr, buf.len)) {
    ...
    pub fn sendTo(self: *Self, buf: []const u8, addr: netip.AddrPort) runtime.SocketError!usize {
        ...
        if (buf.len > max_udp_payload_len) return error.MessageTooLong;
        const encoded = encodeAddr(addr.addr()) catch return error.Unexpected;
        return switch (binding.bkz_lwip_netconn_send_to(conn, buf.ptr, buf.len, &encoded, addr.port())) {
            binding.err_ok => buf.len,
            binding.err_wouldblock, binding.err_mem => {
                setWriteReady(self.state, false);
                return error.WouldBlock;
...
export fn bkz_lwip_runtime_on_event(raw_conn: ?*binding.netconn, event: c_int, _: u16) void {
    const conn = raw_conn orelse return;
    registry_mutex.lock();
    const state = findStateLocked(conn) orelse {
...
fn createRuntime(comptime Socket: type, netconn_type: u32) runtime.CreateError!Socket {
    const conn = binding.bkz_lwip_netconn_new(netconn_type) orelse return error.SystemResources;
    errdefer _ = binding.bkz_lwip_netconn_delete(conn);
...
fn recvTcp(state: *State, conn: *binding.netconn, out: []u8) runtime.SocketError!usize {
    ...
    const rc = binding.bkz_lwip_netconn_recv(conn, &buf);
    switch (rc) {
        binding.err_ok => {
            state.lock();
...
fn deleteState(state: *State) void {
    unregisterStateAndWait(state);
    if (state.conn) |conn| {
        _ = binding.bkz_lwip_netconn_delete(conn);
        state.conn = null;
    }
    state.destroy();
}
...
fn readPendingTcp(state: *State, out: []u8) runtime.SocketError!?usize {
    ...
    const total = binding.bkz_lwip_netbuf_len(pending);
    if (offset >= total) {
        binding.bkz_lwip_netbuf_delete(pending);
        state.lock();
    ...
    const n = binding.bkz_lwip_netbuf_copy(pending, offset, out.ptr, @min(out.len, total - offset));
    state.lock();
...
fn recvUdp(state: *State, conn: *binding.netconn, out: []u8, remote: ?*netip.AddrPort) runtime.SocketError!usize {
    ...
    defer binding.bkz_lwip_netbuf_delete(buf);

    const total = binding.bkz_lwip_netbuf_len(buf);
    const n = binding.bkz_lwip_netbuf_copy(buf, 0, out.ptr, @min(out.len, total));
    if (remote) |addr| {
        var raw_addr: binding.ip_addr = undefined;
        var port: u16 = 0;
        binding.bkz_lwip_netbuf_from_addr(buf, &raw_addr, &port);
...
fn connectConn(conn: *binding.netconn, addr: *const binding.ip_addr, port: u16) runtime.SocketError!void {
    return switch (binding.bkz_lwip_netconn_connect(conn, addr, port)) {
...
fn getAddr(conn: *binding.netconn, local: bool) runtime.SocketError!netip.AddrPort {
    ...
    var port: u16 = 0;
    return switch (binding.bkz_lwip_netconn_get_addr(conn, if (local) 1 else 0, &addr, &port)) {
...
fn applySocketLevelOpt(conn: *binding.netconn, opt: runtime.SocketLevelOption) runtime.SetSockOptError!void {
    switch (opt) {
        .reuse_addr => |enabled| try setOptResult(binding.bkz_lwip_netconn_set_socket_reuse_addr(conn, if (enabled) 1 else 0)),
        .reuse_port => |enabled| {
            if (enabled) return error.Unsupported;
        },
        .broadcast => |enabled| try setOptResult(binding.bkz_lwip_netconn_set_socket_broadcast(conn, if (enabled) 1 else 0)),
        .recv_buffer_size, .send_buffer_size => return error.Unsupported,
    }
}
...
fn registerState(state: *State) void {
    ...
    state.next = registry_head;
    registry_head = state;
}

fn unregisterStateAndWait(state: *State) void {
    registry_mutex.lock();
...
fn findStateLocked(conn: *binding.netconn) ?*State {
