...
const time = struct {
    ...
    const instant = struct {
        pub const Time = glib.time.instant.Time;
        pub const add = glib.time.instant.add;
        pub const sub = glib.time.instant.sub;

        pub fn now() Time {
            return @import("../time.zig").instant.now();
        }
    };
};
...
fn Channel(comptime std: type, comptime T: type) type {
    comptime {
        if (@sizeOf(T) == 0) @compileError("bk.Channel does not support zero-sized element types");
    }
    ...
    return struct {
        ...
        pub fn deinit(self: *Self) void {
            ...
            inner.can_send.deinit();
            inner.can_recv.deinit();
            inner.mutex.deinit();
            inner.send_mutex.deinit();
        ...
        pub fn send(self: *Self, value: T) anyerror!sync.channel.SendResult() {
            if (self.inner.capacity == 0)
                return self.sendUnbuffered(value, null);
            return self.sendBuffered(value, null);
        }

        pub fn sendTimeout(self: *Self, value: T, timeout: time.duration.Duration) anyerror!sync.channel.SendResult() {
            if (self.inner.capacity == 0)
                return self.sendUnbuffered(value, timeout);
            return self.sendBuffered(value, timeout);
        }

        pub fn recv(self: *Self) anyerror!sync.channel.RecvResult(T) {
            if (self.inner.capacity == 0)
                return self.recvUnbuffered(null);
            return self.recvBuffered(null);
        }

        pub fn recvTimeout(self: *Self, timeout: time.duration.Duration) anyerror!sync.channel.RecvResult(T) {
            if (self.inner.capacity == 0)
                return self.recvUnbuffered(timeout);
            return self.recvBuffered(timeout);
        }

        fn sendBuffered(self: *Self, value: T, timeout: ?time.duration.Duration) anyerror!sync.channel.SendResult() {
            self.inner.mutex.lock();
            defer self.inner.mutex.unlock();
        ...
        fn recvBuffered(self: *Self, timeout: ?time.duration.Duration) anyerror!sync.channel.RecvResult(T) {
            self.inner.mutex.lock();
            defer self.inner.mutex.unlock();
        ...
        fn sendUnbuffered(self: *Self, value: T, timeout: ?time.duration.Duration) anyerror!sync.channel.SendResult() {
        ...
        fn recvUnbuffered(self: *Self, timeout: ?time.duration.Duration) anyerror!sync.channel.RecvResult(T) {
            self.inner.mutex.lock();
            defer self.inner.mutex.unlock();
