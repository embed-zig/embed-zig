...
pub fn TestRunner() glib.testing.TestRunner {
    const Runner = struct {
        ...
        fn checkFixture(allocator: std.mem.Allocator) !void {
            ...
            try std.testing.expectEqualStrings("Issue 579 test", spec.info.title);
            ...
            const _sor_AliasedDate = Spec.findNamed(Spec.SchemaOrRef, components.schemas, "AliasedDate") orelse return error.FixtureMismatch;
            switch (_sor_AliasedDate) {
                .reference => |r| {
                    _ = r.ref_path;
                },
                .schema => |sch| {
