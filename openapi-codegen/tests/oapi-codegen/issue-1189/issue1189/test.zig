        ...
        fn checkFixture(allocator: std.mem.Allocator) !void {
            ...
            switch (_sor_test) {
                ...
                .schema => |sch| {
                    ...
                    {
                        const pf = Spec.findNamed(Spec.SchemaOrRef, sch.properties, "fieldA") orelse return error.FixtureMismatch;
                        const pSch = switch (pf) {
                            .schema => |x| x,
                            .reference => return error.FixtureMismatch,
                        };
                        try std.testing.expectEqual(@as(usize, 2), pSch.any_of.len);
                    }
                },
            }
        }
