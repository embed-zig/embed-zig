...
pub fn TestRunner() glib.testing.TestRunner {
    const Runner = struct {
        ...
        fn checkFixture(allocator: std.mem.Allocator) !void {
            ...
            {
                const p_or = Spec.findNamed(Spec.PathItemOrRef, spec.paths, "/v1/test") orelse return error.FixtureMismatch;
            ...
            const _sor_ProblemDetails = Spec.findNamed(Spec.SchemaOrRef, components.schemas, "ProblemDetails") orelse return error.FixtureMismatch;
            switch (_sor_ProblemDetails) {
                .reference => |r| {
                    _ = r.ref_path;
                },
                .schema => |sch| {
