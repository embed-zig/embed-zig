    ...
    const Runner = struct {
        ...
        fn checkFixture(allocator: std.mem.Allocator) !void {
            ...
            try std.testing.expectEqual(@as(usize, 2), spec.paths.len);
            {
                ...
                try std.testing.expectEqual(@as(?[]const u8, null), post_op.operation_id);
            }
            {
                const p_or = Spec.findNamed(Spec.PathItemOrRef, spec.paths, "/pet/1234") orelse return error.FixtureMismatch;
                const p_it = switch (p_or) {
                    .path_item => |x| x,
                    .reference => return error.FixtureMismatch,
                };
