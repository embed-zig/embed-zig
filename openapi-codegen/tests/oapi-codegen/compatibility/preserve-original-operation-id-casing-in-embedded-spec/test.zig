...
pub fn TestRunner() glib.testing.TestRunner {
    const Runner = struct {
        ...
        fn checkFixture(allocator: std.mem.Allocator) !void {
            ...
            try std.testing.expectEqualStrings("my spec", spec.info.title);
            ...
            try std.testing.expectEqual(@as(usize, 1), spec.paths.len);
            {
                ...
                try std.testing.expectEqualStrings("this-is-a-kebabAndCamel_SNAKE", delete_op.operation_id orelse return error.FixtureMismatch);
            }
        }
    };
