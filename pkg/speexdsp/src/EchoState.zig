...
test "speexdsp/unit_tests/echo_state/rejects_invalid_init_arguments" {
    const std = @import("std");
    const testing = std.testing;

    try testing.expectError(error.InvalidArgument, Self.init(0, 160));
    try testing.expectError(error.InvalidArgument, Self.init(160, 0));
}

test "speexdsp/unit_tests/echo_state/rejects_invalid_frame_lengths" {
    const std = @import("std");
    const testing = std.testing;
    ...
    try testing.expectError(error.InvalidArgument, echo.cancellation(rec[0..159], play[0..], out[0..]));
    try testing.expectError(error.InvalidArgument, echo.cancellation(rec[0..], play[0..159], out[0..]));
    try testing.expectError(error.InvalidArgument, echo.cancellation(rec[0..], play[0..], out[0..159]));
    try testing.expectError(error.InvalidArgument, echo.capture(rec[0..159], out[0..]));
    try testing.expectError(error.InvalidArgument, echo.capture(rec[0..], out[0..159]));
    try testing.expectError(error.InvalidArgument, echo.playback(play[0..159]));
}

test "speexdsp/unit_tests/echo_state/rejects_invalid_sampling_rate" {
    const std = @import("std");
    const testing = std.testing;
    ...
    try testing.expectError(error.InvalidArgument, echo.setSamplingRate(0));
}
