...
test "speexdsp/unit_tests/binding/exports_core_symbols" {
    const std = @import("std");
    const testing = std.testing;

    try testing.expect(@sizeOf(spx_int16_t) == 2);
    try testing.expect(@sizeOf(spx_int32_t) == 4);
    try testing.expect(@sizeOf(*SpeexEchoState) > 0);
    try testing.expect(@sizeOf(*SpeexPreprocessState) > 0);
    try testing.expect(@sizeOf(*SpeexResamplerState) > 0);
