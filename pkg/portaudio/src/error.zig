...
test "portaudio/unit_tests/error/maps_known_negative_codes" {
...
test "portaudio/unit_tests/error/returns_null_for_unknown_codes" {
    const std = @import("std");
    const testing = std.testing;
...
test "portaudio/unit_tests/error/classifies_overflow_and_underflow" {
    const std = @import("std");
    const testing = std.testing;
...
test "portaudio/unit_tests/error/check_accepts_success_codes" {
    try check(binding.paNoError);
    try check(binding.paFormatIsSupported);
}

test "portaudio/unit_tests/error/checked_count_accepts_positive_values" {
    const std = @import("std");
    const testing = std.testing;
